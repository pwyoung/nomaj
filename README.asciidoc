...
# Wait a while for the VMs to be created and for Ansible to be installed in them
...
* Using Make files
The above can all be done with the included Makefiles (which are helpful for speeding up dev and remembering commands)
```
# Go the the folder for the project
cd /home/$USER/git/nomaj/examples/k8s-via-kubespray
